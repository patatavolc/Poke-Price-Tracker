...
const FlipCard = ({ cards = [], interval = 3000, className = "" }) => {
  ...
  const cardBack =
    "/images/card-back.png"; // Asegúrate de tener esta imagen en tu carpeta pública
      ...
      setTimeout(() => {
        if (cardRef.current) {
          ...
          requestAnimationFrame(() => {
            if (cardRef.current) {
              cardRef.current.style.transition = "transform 600ms ease-in-out";
            }
          });
        }
      }, 600);
      ...
      >
        ...
        >
          <img
            ...
            onError={(e) => {
              console.error("Error loading image:", currentCard);
              e.target.src =
                "https://via.placeholder.com/245x342?text=Pokemon+Card";
            }}
          />
        </div>
        ...
        >
          <img
            ...
            onError={(e) => {
              // Fallback a una imagen de dorso alternativa
              e.target.src =
                "https://limitlesstcg.s3.us-east-2.amazonaws.com/pokemon/gen/cardback.jpg";
            }}
          />
        </div>
      </div>
